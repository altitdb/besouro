...
public class TestAdditionClassifier extends AbstractClassifier implements Classifier {
	...
	public Episode classify(List<Action> paramActions) {
		...
		if (actions.size() == 2) {
			if (actions.get(0).isTestCreationAction()
					&& actions.get(1).isTestSuccessfullAction()) {
				episode = createTestAdditionEpisode(actions);
			}
		} else if (actions.size() == 4) {
			if (actions.get(0).isTestCreationAction()
					&& actions.get(1).isTestFailureAction()
					&& actions.get(2).isTestCodingAction()
					&& actions.get(3).isTestSuccessfullAction()) {
				episode = createTestAdditionEpisode(actions);
			}
		} else if (actions.size() % 2 == 0) {
